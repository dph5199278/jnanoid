...
public enum Math {
   ...
   */
  private final int MAX_POWER_OF_SQRT2_UNSIGNED = 0xB504F333;
  ...
  public int log2(int x, RoundingMode mode) {
    checkPositive("x", x);
    switch (mode) {
      case UNNECESSARY:
  ...
  private void checkRoundingUnnecessary(boolean condition) {
    if (!condition) {
   ...
   */
  private int lessThanBranchFree(int x, int y) {
  ...
  private void checkPositive(String role, int x) {
    if (x <= 0) {
